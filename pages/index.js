...
export default function Home() {
  ...
  async function loadNFTs() {
    /* 
      @load: provider, token contract, marketplace address, data for nft items
     */
    ...
    //  const provider = new ethers.providers.JsonRpcProvider()
     ...
     const marketContract = new ethers.Contract(nftmarketaddress, SGMarket.abi, provider)
  ...
  async function buyNFT(nft) {
    ...
    const provider = new ethers.providers.Web3Provider(connection)
    const signer = provider.getSigner()
    const contract = new ethers.Contract(nftmarketaddress, SGMarket.abi, signer)
  ...
  if(loadingState === 'loaded' && !nfts.length) return (
    <h1 className="px-20 py-7 text-4xl">No NFTs in marketplace</h1>
  ) 
